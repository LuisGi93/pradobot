...
class EncargadoMensajesGrupales
  ...
  def initialize  #se puede meter aqui lo relacionado con accion_chat si solamente se van a llamar funciones en los hijos de accion_chat
    @grupos=Hash.new  #! tienen que ser instanciadas las acciones asociadas a los chats privados/menus ya que suelen tener estado interno, otra cosa es que se pueda hacer una jearquia
  ...
  def realizar_accion(id_chat, datos_mensaje)
    ...
    case datos_mensaje
      ...
      when /\/dudas$/
        dudas_curso=@grupos[id_chat][:curso].obtener_dudas
