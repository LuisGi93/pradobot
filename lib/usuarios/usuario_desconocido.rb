...
class UsuarioDesconocido < ConexionBD
  ...
  def registrarme_en_el_sistema
    ...
    if @rol == 'profesor'
      @@db.from(:profesor).insert(:id_telegram => @id_telegram, :email =>@email)
      @@db.from(:profesor_moodle).insert(:email =>@email, :token => @token, :id_moodle => @id_moodle )
      ...
      @@db.from(:estudiante).insert(:id_telegram => @id_telegram, :email =>@email)
      @@db.from(:estudiante_moodle).insert(:email =>@email, :token => @token, :id_moodle => @id_moodle )
      @cursos.each{
          |curso|
